...
struct SideNotchShape: Shape {
    ...
    // Handed in rather than read from `NotchLayout`: `path(in:)` is a nonisolated protocol
    // requirement and the layout is main-actor state, so the shape carries its own numbers.
